...
// 메소드 호출부 에서 이벤트 종료를 알 수 있다. 호출하는 방법 3가지.
sayHi(completion: {
    print("2초가 지났다. 1")
})
...
// 상단에 클로저를 사용하는 것처럼 map 또한 사용할 수 있다. 또한 호출 방법도 똑같이 되며 간단하게 줄일 수 있다.(세 가지 방법)
